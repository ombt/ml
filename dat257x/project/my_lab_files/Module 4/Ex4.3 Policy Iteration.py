...
# Policy Iteration calculates the optimal policy for an MDP, given its full definition.  The 
# full definition of an MDP is the set of states, the set of available actions for each 
# state, the set of rewards, the discount factor, and the state/reward transition function.

import test_dp               # required for testing and grading your code
import gridworld_mdp as gw   # defines the MDP for a 4x4 gridworld

# **Implement the algorithm for Policy Iteration**.  Policy Iteration calculates the optimal 
# policy for an MDP by doing repeated steps of policy evaluation and policy improvement.
# 
# A empty function **policy_iteration** is provided below; implement the body of the function 
# to correctly calculate the optimal policy for an MDP.  The function defines 5 parameters - 
# a definition of each parameter is given in the comment block for the function.  For sample 
# parameter values, see the calling code in the cell following the function.
# 
# Note that there is a subtle difference between the algorithm for Policy Evaluation, which 
# assumes the policy is stochastic, and the Policy Evaluation step for the Policy Iteration 
# algorithm, which assumes the policy is deterministic.  This means that you cannot directly 
# call your previous code, but you can reuse large pieces of it for the Policy Evaluation step.
...
def policy_iteration(state_count, gamma, theta, get_available_actions, get_transitions):
    ...
    # insert code here to iterate using policy evaluation and policy improvement 
    # (see Policy Iteration algorithm)
