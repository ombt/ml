                       ...
                       flagged)
{
    ...
    if ((flagged == "Y") || (options$chart)) {
        results_days_flagged <- results
        results_days_flagged$CHART_DATA_VALUE <- results$days_flagged
        results_days_flagged$CHART_DATA_VALUE_TYPE <- "DAYS_FLAGGED"
        #
        return(rbind(results, 
                     results_days_flagged))
    } else {
        return(results)
    }
}
