                       ...
                       flagged)
{
    ...
    if ((flagged == "Y") || (options$chart)) {
        results_mean_pwmvalue <- results
        results_mean_pwmvalue$CHART_DATA_VALUE <- results$mean_pwmvalue
        results_mean_pwmvalue$CHART_DATA_VALUE_TYPE <- "MEAN_PWMVALUE"
        #
        return(rbind(results, 
                     results_mean_pwmvalue))
    } else {
        return(results)
    }
}
...
from (
    ...
    and 
        i.datetimestamplocal < date_parse('<END_DATE>',
                                          '%m/%d/%Y %T') 
    and 
