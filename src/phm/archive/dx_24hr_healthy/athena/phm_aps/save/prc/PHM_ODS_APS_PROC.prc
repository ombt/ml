CREATE OR REPLACE PROCEDURE SVC_PHM_OWNER.PHM_ODS_APS_PROC(V_PROD_ODS_ROUTINE_SK NUMBER,V_RUN_DATE DATE, V_BATCH_NUM VARCHAR2,V_UNIX_ID VARCHAR)
AS
  ...
  IF V_ROUTINE_NAME IS NOT NULL THEN
    ...
    --V_PROCESS_ID := PHM_ALGORITHM_UTILITIES_1.PHM_GET_PROCESS_ID();
     ...
     IF V_RUN_TYPE IS NULL  THEN
        ...
        SELECT MAX(ZIPID) INTO V_END_KEY FROM IDAOWNER.APS_LOG_FILES WHERE PROCESS_START_DATE < SYSDATE - 30 AND PROCESSCODE ='P'; 
        V_RUN_TYPE := 'NEW_RUN';
     END IF;
         ...
         ( V_RUN_TYPE IN ('NEW_RUN') AND V_STATUS NOT  IN ('FAILED') )  THEN
          ...
          IF V_RUN_TYPE = 'RE_RUN' THEN
              ...
              BEGIN
                ...
                DELETE FROM  SVC_PHM_ODS.PHM_ODS_APS_ERRORS WHERE ZIPID BETWEEN V_START_KEY AND V_END_KEY
          ...
          IF V_RUN_TYPE = 'NEW_RUN' THEN
              -- GET START KEY AND END KEY FOR ODS EXECUTION
               BEGIN 
                SELECT MIN(ZIPID),MAX(ZIPID)
                 INTO V_START_KEY,V_END_KEY FROM IDAOWNER.APS_LOG_FILES WHERE ZIPID >  V_END_KEY AND PROCESSCODE  = 'P';
          ...
          IF V_PROCESS_STATUS NOT IN ( 'ERRORED','NO_NEW_DATA') THEN  
              ...
              IF V_START_KEY <= V_END_KEY THEN
                  FOR ER_ROW IN  ( SELECT FD.ZIPID,FD.FILEID as FILEID,FD.LOG_FILE_NAME,E.FILEID AS FILEID1,E.MESSAGE_ID,E.DEVICE_ID,E.PL,E.SN,
                                    E.TIMESTAMP,E.MESSAGE,E.MESSAGE_TYPE,E.LOAD_DATE
                                    FROM IDAOWNER.APS_LOG_FILES F, IDAOWNER.APS_LOG_FILE_DETAILS FD,
                                    (SELECT * FROM IDAOWNER.APS_ERRORS WHERE TIMESTAMP > V_RUN_DATE - 30) E
                                    WHERE F.ZIPID = FD.ZIPID AND  FD.FILEID = E.FILEID AND FD.DEVICE_ID = E.DEVICE_ID
                                    AND F.ZIPID BETWEEN V_START_KEY AND V_END_KEY AND  F.PROCESSCODE ='P')
                                    --and rownum < 11)
                  LOOP
                       BEGIN
                           INSERT INTO SVC_PHM_ODS.PHM_ODS_APS_ERRORS VALUES (V_BATCH_NUM,V_RUN_DATE,ER_ROW.ZIPID,ER_ROW.LOG_FILE_NAME,
                                          ER_ROW.FILEID,ER_ROW.MESSAGE_ID,ER_ROW.DEVICE_ID,ER_ROW.PL,ER_ROW.SN,ER_ROW.TIMESTAMP,
                                          ER_ROW.MESSAGE,ER_ROW.MESSAGE_TYPE,ER_ROW.LOAD_DATE,'SVC_PHM_OWNER',SYSDATE,'SVC_PHM_OWNER',SYSDATE); 
                           NUM_ROWS:= NUM_ROWS + 1;
                       EXCEPTION
                          WHEN OTHERS THEN
                             V_PROCESS_STATUS := 'ERRORED';
                             V_ERROR_MESSAGE := 'NOT ABLE TO INSERT THE DATA INTO PHM_APS_ERRORS FOR THE ZIPID' || ER_ROW.ZIPID||' FILE ID ' ||ER_ROW.FILEID 
                                    ||'ERROR ID' ||ER_ROW.MESSAGE_ID ||' DUE TO ' || SQLERRM;
                             PHM_ALGORITHM_UTILITIES_1.PHM_PROCESS_DETAIL_AUDIT_LOG ( V_PROCESS_ID ,V_RUN_DATE,V_TABLE_NAME,V_START_KEY ,V_END_KEY,V_PROCESS_STATUS,V_ERROR_MESSAGE, NUM_ROWS,V_BATCH_NUM,V_ODS_NAME,V_PROD_ODS_ROUTINE_SK);     
                             PHM_ALGORITHM_UTILITIES_1.PHM_PROCESS_AUDIT_LOG(V_PROCESS_ID,V_PROD_FAMILY,V_PROCESS_TYPE,V_ROUTINE_TYPE,V_ODS_NAME,V_ROUTINE_NAME,V_RUN_MODE,V_PROCESS_STATUS,V_ERROR_MESSAGE,V_RUN_DATE ,SYSDATE,V_BATCH_NUM ,V_UNIX_ID,V_PROD_ODS_ROUTINE_SK );
                             COMMIT;
                             EXIT;
