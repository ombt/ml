...
select 
    ...
    max(rawdata.std_dev_dark_count) as max_std_dev_dark_count
from (
    select
...
select 
    ...
    rawdata.pl,
    max(rawdata.max_completion_date) as flag_date,
    max(rawdata.std_dev_dark_count) as chart_data_value
from (
    select
        upper(trim(r.architect_moduleserial)) as modulesn,
        r.architect_productline as pl,
...
group by
    ...
    rawdata.pl
having
    max(rawdata.std_dev_dark_count) is not null
and
...
post_flagged_processing <- function(flagged_results, 
                                    ...
                                    test_period)
{
    flagged_results$CHART_DATA_VALUE <- flagged_results$MAX_STD_DEV_DARK_COUNT
    #
    for (irec in 1:nrow(flagged_results)) {
        modulesn <- flagged_results[irec, "MODULESN"]
        #
        if (grepl("^I1SR", modulesn)) {
             flagged_results[irec, "IHN_LEVEL3_DESC"] <- "I1S DARK COUNT SD > 100"
        } else if (grepl("^ISR", modulesn)) {
             flagged_results[irec, "IHN_LEVEL3_DESC"] <- "I2S DARK COUNT SD > 100"
