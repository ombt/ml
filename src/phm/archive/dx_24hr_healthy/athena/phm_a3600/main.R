...
select distinct
    upper(trim(ac.a3600_serialnumber)) as modulesn,
    ac.a3600_productline as pl
from
    dx.dx_a3600_counter ac
where
    ac.a3600_serialnumber is not null
and
    '<MODULESN_START_DATE>' <= ac.transaction_date
and 
    ac.transaction_date < '<MODULESN_END_DATE>'"
#
                                     ...
                                     threshold_data_days)
{
    ...
    for (idr in 1:nrow(data_results)) {
        ...
        #
        write_data(dr_record, "debug_error_count_record_data.csv")
                                    ...
                                    threshold_data_days)
{
    ...
    for (idr in 1:nrow(data_results)) {
        dr_record <- data_results[idr, ]
        #
        write_data(dr_record, "debug_percentage_record_data.csv")
        write_data(dr_record, "debug_all_data.csv", TRUE)
        #
        if (nchar(dr_record[1,"A3600_SERIALNUMBER_UC"]) <= 0) {
            next
                                       ...
                                       threshold_data_days)
{
    ...
    for (idr in 1:nrow(data_results)) {
        dr_record <- data_results[idr, ]
        #
        write_data(dr_record, "debug_sd_low_volume_record_data.csv")
        write_data(dr_record, "debug_all_data.csv", TRUE)
        #
        if (nchar(dr_record[1,"A3600_SERIALNUMBER_UC"]) <= 0) {
            next
                                        ...
                                        threshold_data_days)
{
    ...
    for (idr in 1:nrow(data_results)) {
        dr_record <- data_results[idr, ]
        #
        write_data(dr_record, "debug_sd_high_volume_record_data.csv")
        write_data(dr_record, "debug_all_data.csv", TRUE)
        #
        if (nchar(dr_record[1,"A3600_SERIALNUMBER_UC"]) <= 0) {
            next
...
#####################################################################
#
# overwrite function to run algorithm for a set of parameters
#
                          ...
                          test_period)
{
    ...
    healthy_flag_date <- test_period["HEALTHY_FLAG_DATE","VALUE"]
    #
    ...
    threshold_number      <- as.integer(error_count)
    threshold_number_desc <- threshold_description
    threshold_number_unit <- as.integer(thresholds_days)
    ...
    if (algorithm_type == "ERROR_COUNT") {
        ...
        write_data(data_results, "debug_error_count_data.csv")
        ...
        names(data_results) <- toupper(names(data_results))
        write_data(data_results, "debug_percentage_data.csv")
        write_data(data_results, "debug_all_data.csv", TRUE)
        #
        ...
        names(data_results) <- toupper(names(data_results))
        write_data(data_results, "debug_sd_high_volume_data.csv")
        write_data(data_results, "debug_all_data.csv", TRUE)
        #
        ...
        names(data_results) <- toupper(names(data_results))
        write_data(data_results, "debug_sd_low_volume_data.csv")
        write_data(data_results, "debug_all_data.csv", TRUE)
        #
    ...
    if (nrow(final_results) > 0) {
        ...
        #
        final_results$FLAG_DATE <- 
            gsub("\\.[0-9][0-9]*","", final_results$FLAG_DATE)
        final_results$FLAG_DATE <- 
            gsub("[^0-9]","", final_results$FLAG_DATE)
    }
    #
