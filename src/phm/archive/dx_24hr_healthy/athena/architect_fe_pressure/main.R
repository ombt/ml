#
# Architect FE Pressure Combined
#
#####################################################################
#
...
from (
    ...
    from (
        select 
            ...
            rawdata.pl,
            rawdata.pipetter,
            sum(case when rawdata.avg_fep >  <ARCH_FEP_THRESHOLD_NUMBER>
                     then 1
                     else 0
...
where
    final.fep_gt27000_cnt >= <ARCH_FEP_THRESHOLD_NUMBER_UNIT>
and
    final.pipetter = '<ARCH_FEP_THRESHOLD_NAME>'
order by
    final.pl,
    final.modulesn,
    final.pipetter"
...
select
    ...
    final.modulesn,
    final.pipetter,
    final.fep_gt27000_cnt as chart_data_value,
    final.flag_date
from (
    select
        derived.pl,
...
post_flagged_processing <- function(flagged_results, 
                                    ...
                                    test_period)
{
    flagged_results$CHART_DATA_VALUE <- flagged_results$FEP_GT27000_CNT
    #
