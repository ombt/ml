...
select 
    ...
    max(rawdata.std_dev_dark_count) as std_dev_dark_count
from (
    select
...
post_flagged_processing <- function(flagged_results, 
                                    ...
                                    test_period)
{
    flagged_results$CHART_DATA_VALUE <- flagged_results$STD_DEV_DARK_COUNT
    #
    for (irec in 1:nrow(flagged_results)) {
        modulesn <- flagged_results[irec, "MODULESN"]
        #
        if (grepl("^I1SR", modulesn)) {
             flagged_results[irec, "IHN_LEVEL3_DESC"] <- "i1SR Dark Count SD >100"
        } else if (grepl("^ISR", modulesn)) {
             flagged_results[irec, "IHN_LEVEL3_DESC"] <- "i2SR Dark Count SD >100"
