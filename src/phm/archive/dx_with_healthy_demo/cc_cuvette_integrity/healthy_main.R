...
select
    eval2.moduleserialnumber as modulesn,
    eval2.total_samples as sample_count,
    case when (eval2.total_samples >= ((eval2.max_cuv_num-eval2.min_cuv_num+1)*<MINIMUM_SAMPLE_COUNT>))
    then 1
    else 0
...
from (
    select
        eval.moduleserialnumber,
        min(eval.cuvettenumber) as min_cuv_num,
        max(eval.cuvettenumber) as max_cuv_num,
        sum(eval.num_sampevents_percuv) as total_samples
    from (
        select
            raw.moduleserialnumber,
            raw.cuvettenumber,
...
master_list_query_template <- "
        select
distinct raw.moduleserialnumber as modulesn
        from (
select
    r.moduleserialnumber,
    r.testid,
...
inner join 
    dx.dx_210_alinity_c_ccdispensepm dpm
on 
    '<MASTER_LIST_START_DATE>' <= dpm.transaction_date
and 
    dpm.transaction_date < '<MASTER_LIST_END_DATE>'
and
    r.moduleserialnumber = dpm.moduleserialnumber
and
    r.scmserialnumber = dpm.scmserialnumber
...
inner join
    dx.dx_214_alinity_ci_ccsampledispensepcidata sdp
on
    '<MASTER_LIST_START_DATE>' <= sdp.transaction_date
and 
    sdp.transaction_date < '<MASTER_LIST_END_DATE>'
and
    sdp.scmserialnumber = dpm.scmserialnumber
and 
    dpm.datetimestamplocal
...
and 
    sdp.replicatenumber = dpm.replicatenumber
where 
    '<MASTER_LIST_START_DATE>' <= r.transaction_date
and 
    r.transaction_date < '<MASTER_LIST_END_DATE>'
and 
    r.cuvettenumber is not null
and 
    r.cuvettenumber 
                            ...
                            flagged)
{
    ...
    #
    flagged <- with(flagged_results, ((NUM_CUVETTES_MAX >= 1) &
                                      (NUM_SAMPEVENTS_PERCUV_GT_20 >= 1) &
                                      (NUM_GT20000_GT20PERC_SAMPEVENTS >= 1)))
    #
    return(flagged_post_processing(flagged_results, flagged))
}
#
#####################################################################
#
