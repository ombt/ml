CREATE OR REPLACE 
PROCEDURE SVC_PHM_OWNER.PHM_DARK_COUNT(V_ALG_NUM   NUMBER,
                                       V_RUN_DATE  DATE,
...
/******************************************************************************
   ...
   3.0        10/03/2017  Siva Chandaluri     Updated the procedure to use instrumentlisting for 
                                              instrument/customer/country info and populate 
...
IS 
    SELECT  
        ...
        CASE WHEN IA.MODULESNDRM like 'I1SR%' then 'I1SR'
             WHEN IA.MODULESNDRM like 'ISR%' then 'I2SR'
             WHEN IA.MODULESNDRM like 'I20%' then 'I2000'
             ElSE IA.MODULESNDRM 
             END MODULETYPE
    FROM 
        ...
        LOOP
            ...
            LOOP
                BEGIN
                    ...
                    EXCEPTION 
                    WHEN OTHERS THEN /* TO CATCH AL TEH EXCPETIONS WHILE GATHERING THE DEVICE INFO */
                        V_PROCESS_STATUS := 'ERRORED';
    ...
    LOOP
        ...
        LOOP
            ...
            BEGIN  /* PROCESS ERROR CONDITIONS FOR EACH DEVICE */
              FOR X IN ( SELECT * FROM PHM_DC_DATA WHERE BATCH_NUM = V_BATCH_NUM AND RUN_DATE = V_RUN_DATE 
                         AND MODULESNDRM = DD.MODULESNDRM AND PHM_THRESHOLDS_SK = AP.PHM_THRESHOLDS_SK ORDER BY COMPLETIONDATE)
              LOOP
                ...
                IF X.DEVICE_VALUE >= AP.THRESHOLD_NUMBER THEN /* VERYFING THE NUMBER OF ERRORS AGAINST THRESHOLD ERROR COUNT */
                  ...
                  IF NUM_DAYS >= AP.THRESHOLD_NUMBER_UNIT THEN /* VERYFING THE NUMBER OF DAYS AGAINST THRESHOLD ERROR DAYS */
                     FLAG := 'YES';
                     VIHN4_CALL_MESSAGE := LOWER(SUBSTR(DD.MODULETYPE,1,2)) || 'SR ' || AP.THRESHOLD_ALERT;
                ...
                PHM_ALGORITHM_UTILITIES_1.PHM_ALGORITHM_OUTPUT_IN_EXP_PL(DD.CUSTOMERNAME,DD.CUSTOMERNUMBER,X.DEVICEID,X.MODULESNDRM,DD.COUNTRYNAME,DD.AREA,
                   V_ALG_NUM,AP.PHM_THRESHOLDS_SK,X.COMPLETIONDATE,X.DEVICE_VALUE,FLAG,VIHN4_CALL_MESSAGE,'',VALGNAME,'',V_BATCH_NUM,AP.PHM_PATTERNS_SK,V_RUN_DATE,V_PROCESS_ID, V_FLAGGED_PL, V_FLAGGED_EXP_CODE);
                V_INSERT_COUNT := V_INSERT_COUNT + 1;
