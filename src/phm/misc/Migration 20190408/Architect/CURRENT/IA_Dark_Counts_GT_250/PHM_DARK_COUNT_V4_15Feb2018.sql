CREATE OR REPLACE 
PROCEDURE 
SVC_PHM_OWNER.PHM_DARK_COUNT(V_ALG_NUM NUMBER,
                             V_RUN_DATE DATE,
...
IS 
    SELECT  
        ...
        CASE WHEN IA.MODULESNDRM like 'I1SR%' then 'I1SR'
             ...
             ElSE IA.MODULESNDRM END MODULETYPE
    FROM 
        ...
        LOOP
            ...
            LOOP
                ...
                BEGIN
                    ...
                    IF AP.THRESHOLD_TYPE = 'SD' 
                    THEN /* INSERT SD INTO DEVICE_VALUE COLUMN*/
                    ...
                    IF AP.THRESHOLD_TYPE = 'AVG' 
                    THEN /* INSERT AVG INTO DEVICE_VALUE COLUMN*/
                        INSERT INTO 
                            PHM_DC_DATA VALUES (V_RUN_DATE,
                                                V_BATCH_NUM,
                                                AP.PHM_THRESHOLDS_SK,
                                                DS.DEVICE_ID,
                                                DS.MODULE_SN, 
                    ...
                    EXCEPTION 
                    WHEN OTHERS 
                    THEN /* TO CATCH AL TEH EXCPETIONS WHILE GATHERING THE DEVICE INFO */
                        V_PROCESS_STATUS := 'ERRORED';
        ...
        LOOP
            ...
            LOOP
                ...
                LOOP
                    ...
                    IF X.DEVICE_VALUE >= AP.THRESHOLD_NUMBER 
                    THEN /* VERYFING THE NUMBER OF ERRORS AGAINST THRESHOLD ERROR COUNT */
                        NUM_DAYS := NUM_DAYS + 1;
                        IF NUM_DAYS >= AP.THRESHOLD_NUMBER_UNIT 
                        THEN /* VERYFING THE NUMBER OF DAYS AGAINST THRESHOLD ERROR DAYS */
                            FLAG := 'YES';
                ...
                EXCEPTION 
                WHEN OTHERS 
                THEN  /* TO CATCH ANY ERROR THAT HAPPENS DURING THE ALGORITHMS CALCULATIONS AND MAKING THE RUN AS ERRORD IN AUDIT TABLES  */
                    V_PROCESS_STATUS := 'ERRORED';
