CREATE OR REPLACE PROCEDURE SVC_PHM_OWNER.PHM_DARK_COUNT(V_ALG_NUM NUMBER,V_RUN_DATE DATE,V_BATCH_NUM VARCHAR2, V_UNIX_ID VARCHAR2)
IS
/******************************************************************************
   NAME:       PHM_DARK_COUNT
...
CURSOR DEVICE_STATS (V_MODULESNDRM VARCHAR2,V_START_DATE DATE,DAYS_NUM NUMBER,V_END_DATE DATE)
IS
SELECT R.DEVICEID DEVICE_ID, R.MODULESNDRM MODULE_SN,TRUNC (R.COMPLETIONDATE) TEST_COMPLETION_DATE, MAX(R.COMPLETIONDATE) MAX_COMP_DATE,
MAX(TO_NUMBER(R.DARKCOUNT)) MAX_DARK_COUNT_BY_DAY, STDDEV (R.DARKCOUNT) STD, COUNT (R.DARKCOUNT) COUNT_DARK_COUNT_BY_DAY,
AVG (TO_NUMBER (R.DARKCOUNT)) AVG_DARK_COUNT FROM SVC_PHM_ODS.PHM_ODS_RESULTS_IA R WHERE MODULESNDRM = V_MODULESNDRM AND DARKCOUNT IS NOT NULL
AND COMPLETIONDATE BETWEEN V_START_DATE - DAYS_NUM AND V_END_DATE GROUP BY DEVICEID,MODULESNDRM,TRUNC(COMPLETIONDATE) ORDER BY 1,2;
...
CURSOR DEVICE_AND_DATE_LIST
IS 
SELECT  IA.DEVICEID, IA.MODULESNDRM, MAX (IL.PL) PL, MAX (IL.CUSTOMER_NUM) CUSTOMERNUMBER,
...
FROM SVC_PHM_ODS.PHM_ODS_RESULTS_IA IA, INSTRUMENTLISTING IL, PHM_COUNTRY PC
WHERE IA.COMPLETIONDATE > TRUNC(SYSDATE) - 3 AND IA.RUN_DATE = V_RUN_DATE AND IA.BATCH_NUM = V_BATCH_NUM AND IL.SN = IA.MODULESNDRM AND PC.COUNTRY_CODE = IL.COUNTRY_CODE
...
AND PP.PHM_ALGORITHM_DEFINITIONS_SK = PTT.PHM_ALGORITHM_DEFINITIONS_SK AND PP.PHM_ALGORITHM_DEFINITIONS_SK = VALGNUM;
     ...
     IF V_PROCESS_STATUS <>  'ERRORED' THEN
        ...
        LOOP
          ...
          LOOP
            ...
            FOR DS IN  DEVICE_STATS (DD.MODULESNDRM,DD.CUR_MIN_DATE,AP.THRESHOLD_DATA_DAYS,DD.CUR_MAX_DATE) /* EACH DEVICE STATICS TO PHM_DC_DATA*/
            LOOP
              BEGIN
                ...
                IF AP.THRESHOLD_TYPE = 'SD' THEN /* INSERT SD INTO DEVICE_VALUE COLUMN*/
                   INSERT INTO PHM_DC_DATA VALUES (V_RUN_DATE,V_BATCH_NUM,AP.PHM_THRESHOLDS_SK,DS.DEVICE_ID,DS.MODULE_SN, AP.THRESHOLD_TYPE,
                                                DS.MAX_COMP_DATE,DS.STD,0,VALGNAME, SYSDATE, V_UNIX_ID);
                END IF;
                    
                IF AP.THRESHOLD_TYPE = 'AVG' THEN /* INSERT AVG INTO DEVICE_VALUE COLUMN*/
                    INSERT INTO PHM_DC_DATA VALUES (V_RUN_DATE,V_BATCH_NUM,AP.PHM_THRESHOLDS_SK,DS.DEVICE_ID,DS.MODULE_SN, AP.THRESHOLD_TYPE,
                                                DS.MAX_COMP_DATE,DS.AVG_DARK_COUNT,0,VALGNAME, SYSDATE, V_UNIX_ID);
                END IF;   
