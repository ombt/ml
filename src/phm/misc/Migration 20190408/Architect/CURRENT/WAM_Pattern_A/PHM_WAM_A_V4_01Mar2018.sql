CREATE OR REPLACE PROCEDURE SVC_PHM_OWNER.PHM_WAM_A(V_ALG_NUM NUMBER,V_RUN_DATE DATE,V_BATCH_NUM VARCHAR2, V_UNIX_ID VARCHAR2)
IS

CURSOR DEVICE_STATS (V_MODULESNDRM VARCHAR2,V_START_DATE DATE,V_END_DATE DATE)
IS
SELECT DEVICEID,MODULESNDRM,TRUNC(EVENTDATE) DT,WASHZONEID  WZ_ID FROM SVC_PHM_ODS.PHM_ODS_WASHASPIRATIONS
...
IA.CUSTOMERNAME NOT LIKE '%ABBOTT%' AND  IA.CUSTOMERNAME NOT LIKE '%Flextronics%'
...
SELECT  WAM.COUNTRY,    WAM.AREAREGION,    WAM.CUSTOMER,    WAM.CUSTOMERNUMBER,    WAM.SN, WAM.PL,   WAM.WZPROBE,    WAM.FLAGDATE, WAM.DEVICE_VALUE,
    WAM.FLAGA,   '' AS IHN_LEVEL3_DESC,    SYSDATE
...
ABC.FLAGDATEA AS FLAGDATE,  ABC.DEVICE_VALUE AS DEVICE_VALUE, ABC.FLAGA
 FROM  (SELECT  A.COUNTRY AS COUNTRY, A.AREAREGION AS AREAREGION,  A.CUSTOMER_NAME AS CUSTOMER, A.CUSTOMERNUMBER   AS CUSTOMERNUMBER, A.SN  AS SN, A.PL AS PL,
 A.WZPROBEA AS WZPROBE,
 A.FLAGDATEA AS FLAGDATEA, A.MAXTEMP AS DEVICE_VALUE, CASE WHEN A.FLAGA = 'YES'  THEN 'YES'  ELSE 'NO' END   AS FLAGA
 FROM  (
SELECT  RAWA.COUNTRY,  RAWA.AREAREGION,  RAWA.CUSTOMER_NAME,   RAWA.CUSTOMERNUMBER,
        RAWA.SN, RAWA.PL, RAWA.WZPROBE AS WZPROBEA, RAWA.FLAGA AS FLAGA, RAWA.FLAGDATEA AS FLAGDATEA, RAWA.MAXTEMP
FROM   (SELECT IA.PRODUCTLINE AS PL, IA.AREA,IA.COUNTRYNAME COUNTRY,IA.AREAREGION,IA.CUSTOMERNAME AS CUSTOMER_NAME,IA.CUSTOMERNUMBER CUSTOMERNUMBER,
WZA.MODULESNDRM AS SN,WZA.WASHZONEID || '.' || WZA.POSITION AS WZPROBE,WZA.EVENTDATE AS FLAGDATEA,WZA.MAXTEMP AS MAXTEMP,
CASE WHEN WZA.MAXTEMP > V_THRESHOLD_TEMP AND
                    LAG (WZA.MAXTEMP) OVER (PARTITION BY WZA.MODULESNDRM, WZA.WASHZONEID, WZA.POSITION ORDER BY WZA.EVENTDATE) > V_THRESHOLD_TEMP AND
                    LAG (WZA.MAXTEMP, 2) OVER (PARTITION BY WZA.MODULESNDRM, WZA.WASHZONEID, WZA.POSITION ORDER BY WZA.EVENTDATE) > V_THRESHOLD_TEMP AND
                    LAG (WZA.MAXTEMP, 3) OVER (PARTITION BY WZA.MODULESNDRM, WZA.WASHZONEID, WZA.POSITION ORDER BY WZA.EVENTDATE) > V_THRESHOLD_TEMP AND
                    LAG (WZA.MAXTEMP, 4) OVER (PARTITION BY WZA.MODULESNDRM, WZA.WASHZONEID, WZA.POSITION ORDER BY WZA.EVENTDATE) > V_THRESHOLD_TEMP
                    THEN 'YES'  ELSE 'NO' END AS FLAGA
FROM( SELECT WA.MODULESNDRM,WA.EVENTDATE,  WA.WASHZONEID - 1 AS WASHZONEID,'1' AS POSITION,WA.POSITION1 AS REPLICATEID,
        CASE WHEN WA.POSITION1 = LAG (WA.POSITION1) OVER (ORDER BY WA.MODULESNDRM, WA.POSITION1, WA.WASHZONEID, WA.EVENTDATE) AND
           WA.WASHZONEID = LAG (WA.WASHZONEID) OVER (ORDER BY WA.MODULESNDRM, WA.POSITION1, WA.WASHZONEID, WA.EVENTDATE) AND
           WA.EVENTDATE - 10 /(24*60*60) < LAG (WA.EVENTDATE) OVER (ORDER BY WA.MODULESNDRM, WA.POSITION1, WA.WASHZONEID, WA.EVENTDATE)
           THEN 'Probe 1 Second Temp'
        ELSE 'Probe 1 First Temp' END
        AS PIP_ORDER,WA.MAXTEMPPOSITION1/1000 MAXTEMP
        FROM PHM_ODS_WASHASPIRATIONS WA
        WHERE  WA.POSITION1 > 0 AND WA.EVENTDATE >= TRUNC(SYSDATE - 1) AND WA.EVENTDATE < TRUNC(SYSDATE)
        AND WA.MODULESNDRM IN (SELECT DISTINCT MODULESNDRM FROM PHM_ODS_WASHASPIRATIONS WHERE BATCH_NUM=V_BATCH_NUM AND EVENTDATE >= TRUNC(SYSDATE - 1) AND EVENTDATE < TRUNC(SYSDATE))
        --   AND wa.modulesndrm = ('ISR52133')
    UNION ALL
        SELECT WA.MODULESNDRM,WA.EVENTDATE, WA.WASHZONEID - 1 AS WASHZONEID, '2' AS POSITION,WA.POSITION2 AS REPLICATEID,
        'Probe 2' AS PIP_ORDER, WA.MAXTEMPPOSITION2/1000 MAXTEMP
        FROM  PHM_ODS_WASHASPIRATIONS WA
        WHERE WA.POSITION2 > 0 AND WA.EVENTDATE >= TRUNC(SYSDATE - 1) AND WA.EVENTDATE < TRUNC(SYSDATE)
        AND WA.MODULESNDRM IN (SELECT DISTINCT MODULESNDRM FROM PHM_ODS_WASHASPIRATIONS WHERE BATCH_NUM=V_BATCH_NUM AND EVENTDATE >= TRUNC(SYSDATE - 1) AND EVENTDATE < TRUNC(SYSDATE))
        -- AND  WA.modulesndrm = ('ISR52133')
    UNION ALL
        SELECT WA.MODULESNDRM,WA.EVENTDATE,  WA.WASHZONEID - 1 AS WASHZONEID,'3' AS POSITION,WA.POSITION3 AS REPLICATEID,
        CASE WHEN WA.POSITION3 = LAG (WA.POSITION3) OVER (ORDER BY WA.MODULESNDRM, WA.POSITION3, WA.WASHZONEID, WA.EVENTDATE) AND
           WA.WASHZONEID = LAG (WA.WASHZONEID) OVER (ORDER BY WA.MODULESNDRM, WA.POSITION3, WA.WASHZONEID, WA.EVENTDATE) AND
           WA.EVENTDATE - 10 /(24*60*60) < LAG (WA.EVENTDATE) OVER (ORDER BY WA.MODULESNDRM, WA.POSITION3, WA.WASHZONEID, WA.EVENTDATE)
        THEN 'Probe 3 Second Temp' ELSE 'Probe 3 First Temp' END AS PIP_ORDER, WA.MAXTEMPPOSITION3/1000 MAXTEMP
        FROM PHM_ODS_WASHASPIRATIONS WA
        WHERE WA.POSITION3 > 0 AND WA.EVENTDATE >= TRUNC(SYSDATE - 1) AND WA.EVENTDATE < TRUNC(SYSDATE)
        AND WA.MODULESNDRM IN (SELECT DISTINCT MODULESNDRM FROM PHM_ODS_WASHASPIRATIONS WHERE BATCH_NUM=V_BATCH_NUM AND EVENTDATE >= TRUNC(SYSDATE - 1) AND EVENTDATE < TRUNC(SYSDATE))
        --  AND   modulesndrm = ('ISR52133')
) WZA INNER JOIN IDAOWNER.IDAMODULEINFORMATION IA
ON  WZA.MODULESNDRM = IA.MODULESN AND
IA.CREATEDATE = (SELECT MAX(CREATEDATE) from IDAOWNER.IDAMODULEINFORMATION where MODULESN = IA.MODULESN AND CREATEDATE <= SYSDATE) AND-- AND PRODUCTLINE<>'XXX') AND
WZA.EVENTDATE > IA.EFFECTIVEFROMDATE AND   WZA.EVENTDATE < IA.EFFECTIVETODATE
WHERE NOT WZA.PIP_ORDER = 'Probe 3 Second Temp' AND NOT WZA.PIP_ORDER = 'Probe 1 First Temp' AND
IA.AREA IS NOT NULL AND IA.CUSTOMERNAME NOT LIKE '%ABBOTT%' AND IA.CUSTOMERNAME NOT LIKE '%Flextronics%'   ) RAWA
) A
...
BEGIN
 ...
 V_PROCESS_ID := PHM_ALGORITHM_UTILITIES_1.PHM_GET_PROCESS_ID();
  ...
  LOOP
    IF I.PARAMETER_NAME = 'ERROR_CODE_VALUE' THEN V_THRESHOLD_TEMP  :=  I.PARAMETER_VALUES;  END IF;
     ...
     IF V_PROCESS_STATUS <>  'ERRORED' THEN
        ...
        INSERT INTO PHM_WAM_OUTPUT_1(CUSTOMER, CUSTOMERNUMBER, DEVICE_ID, SN, COUNTRY, REGION, PHM_ALGORITHM_DEFINITIONS_SK, PHM_THRESHOLDS_SK, FLAGDATE
                              ...
                              VALUES(F.CUSTOMER, F.CUSTOMERNUMBER, NULL, F.SN, F.COUNTRY, F.AREAREGION, V_ALG_SK, V_THRESHOLD_SK_A, F.FLAGDATE
                                        , F.DEVICE_VALUE, F.WZPROBE, F.FLAGA, V_IHN4_CALL_MESSAGE, NULL, VALGNAME, sysdate, NULL, V_BATCH_NUM, V_ALG_NUM, F.PL);
         ...
         BEGIN
            ...
            IF X.CNT = 1 THEN
               BEGIN
                  ...
                  WHERE W.WZPROBE IS NOT NULL AND  PHM_PATTERNS_SK = V_ALG_NUM AND W.FLAGA = 'YES' AND W.SN = X.SN
                 ...
                 UPDATE  PHM_WAM_OUTPUT_1 SET IHN_LEVEL3_DESC = IHN3_MSG  WHERE SN = X.SN AND TRUNC(FLAGDATE) = TRUNC(X.FLAGDATE)
                 AND  PHM_PATTERNS_SK = V_ALG_NUM  AND BATCH_NUM = V_BATCH_NUM  AND ( FLAGA = 'YES');
