CREATE OR REPLACE PROCEDURE SVC_PHM_OWNER.PHM_CC_CUVETTE_CODE3_PROC (
   V_ALG_NUM      NUMBER,
   V_RUN_DATE     DATE,
...
--             AND MODULESNDRM = 'C1600308'
                      ...
                      (SELECT DISTINCT LOWER (SN)
                         FROM (SELECT C.SN
                                 FROM (SELECT F.DEVICE_ID,
                                              ...
                                              || ' - '
                                                 AS Msg
                                         FROM SVC_PHM_ODS.PHM_ODS_CC_CUVETTE_IMPACTED F
                                              ...
                                              AND F.SN IN
                                                     (SELECT SN
                                                        FROM (  SELECT DEVICE_ID,
                                                                       ...
                                                                       COUNT (
                                                                          *)
                                                                  FROM SVC_PHM_ODS.PHM_ODS_CC_CUVETTE_IMPACTED
                                                                 WHERE     ERRORPCT > 10 * 2
                                                                       AND TYPE = 'E25'
                                                                       AND SN LIKE'C4%'
                                                                       AND BATCH_NUM = V_BATCH_NUM AND RUN_DATE = V_RUN_DATE
   ...
   IS
      SELECT N.DEVICE_ID AS DEVICEID,
             N.SN SERIAL_NUM,
             PL,
             CUSTOMER_NUMBER,
             CUSTOMER_NAME,
        ...
        FROM (  SELECT MAX (SN) SN,
             ...
             PHM_COUNTRY PC,
             (SELECT C.*
                FROM (SELECT distinct F.DEVICE_ID,
                             LOWER (F.SN) AS SN,
                             -- 'Cuvette ' || F.CUVETTE || ' - Cuvette Broken' AS Msg
                             'NA' as Msg
                        FROM SVC_PHM_ODS.PHM_ODS_CC_CUVETTE_IMPACTED F
                       WHERE     F.ERRORPCT > 10 * 2
                             AND F.TYPE = 'E25'
                             AND F.BATCH_NUM = V_BATCH_NUM AND F.RUN_DATE = V_RUN_DATE
                             AND F.SN IN (SELECT SN
                                            FROM (  SELECT DEVICE_ID,
                                                           SN,
                                                           COUNT (*)
                                                      FROM SVC_PHM_ODS.PHM_ODS_CC_CUVETTE_IMPACTED
   ...
   IS
      SELECT E.sn,
        ...
        FROM (SELECT *
                FROM SVC_PHM_ODS.PHM_ODS_CC_CUVETTE_IMPACTED F
               WHERE F.TYPE = 'E24' AND F.ERROR_COUNT IS NOT NULL AND F.BATCH_NUM = V_BATCH_NUM AND F.RUN_DATE = V_RUN_DATE) E,
             (SELECT *
                FROM SVC_PHM_ODS.PHM_ODS_CC_CUVETTE_IMPACTED F
               WHERE F.TYPE = 'All' AND F.Total_COUNT IS NOT NULL AND F.BATCH_NUM = V_BATCH_NUM AND F.RUN_DATE = V_RUN_DATE) A
        ...
        FROM (SELECT *
             ...
             (SELECT *
               ...
               WHERE F.TYPE = 'All' AND F.Total_COUNT IS NOT NULL
                 AND F.BATCH_NUM = V_BATCH_NUM AND F.RUN_DATE = V_RUN_DATE) A
       WHERE E.sn = A.sn AND E.cuvette = A.Cuvette;
...
*/   
   ...
   DELETE FROM SVC_PHM_ODS.PHM_ODS_CC_CUVETTE_IMPACTED WHERE DATE_CREATED < SYSDATE -2;
   COMMIT;
   ...
   LOOP
      ...
      THEN
         ...
         -- F24 >15000
         INSERT INTO SVC_PHM_ODS.PHM_ODS_CC_CUVETTE_IMPACTED (DEVICE_ID,
                                                              Module_Number,
                                                              SN,
                                                              Cuvette,
                                                              TYPE,
   ...
   LOOP
      ...
      UPDATE SVC_PHM_ODS.PHM_ODS_CC_CUVETTE_IMPACTED
...
EXCEPTION
   ...
   THEN
      ...
      PHM_ALGORITHM_UTILITIES_1.PHM_PROCESS_AUDIT_LOG (V_PROCESS_ID,
                                                       ...
                                                       V_ALG_NUM);
      COMMIT;
END PHM_CC_CUVETTE_CODE3_PROC;


-- Assigning the grants to SVC_PHM_CONNECT user for ETL process to pick
grant execute on SVC_PHM_OWNER.PHM_CC_CUVETTE_CODE3_PROC to SVC_PHM_CONNECT;