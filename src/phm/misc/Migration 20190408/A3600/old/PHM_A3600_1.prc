CREATE OR REPLACE PROCEDURE SVC_PHM_OWNER.PHM_A3600_1( V_ALG_NUM NUMBER,V_RUN_DATE DATE, V_BATCH_NUM VARCHAR2,V_UNIX_ID VARCHAR2)
AS
          ...
          FROM (SELECT tp.phm_patterns_sk,
                  ...
                  FROM phm_threshold_parameter tp,
                       ...
                       (SELECT phm_patterns_sk, issue_description
                         ...
                         WHERE pai.phm_algorithm_definitions_sk = vn_ALG_NUM) ihn,
                         (select phm_patterns_sk, phm_thresholds_sk from phm_thresholds pt where pt.phm_algorithm_definitions_sk = vn_ALG_NUM) thr
   ...
   IS
        ...
        SELECT ASI.DEVICEID,
         ...
         WHERE     AE.LAYOUT_NODES_ID = N.LAYOUT_NODES_ID
               ...
               AND AE.COMPLETIONDATE BETWEEN V_START_DATE  - V_DATA_DAYS + 1 AND V_END_DATE
            ...
            BEGIN
               ...
               INSERT INTO SVC_PHM_ODS.PHM_A3600_TEMP_ERROR
                    VALUES (
                              ...
                              0,
                              TO_DATE (
                                    TO_CHAR (AEC.COUNTER_DATE, 'DDMMYYYY')
                                 || '000001',
                                 'DDMMYYYYHH24MISS'),
                              0,
                              'A3600-SYS',
...
LOOP   
DBMS_OUTPUT.PUT_LINE ('ALGORITHM_TYPE ='|| Z.ALGORITHM_TYPE||', Z.PATTERN_DESCRIPTION='||Z.PATTERN_DESCRIPTION);

   --Error type
   IF Z.ALGORITHM_TYPE = 'ERROR_COUNT'
   THEN
      DBMS_OUTPUT.PUT_LINE ('Z.PATTERN_DESCRIPTION =' || '  ' || Z.PATTERN_DESCRIPTION||',Z.PHM_THRESHOLDS_SK='||Z.PHM_THRESHOLDS_SK||', V_BATCH_NUM='||V_BATCH_NUM||',V_RUN_DATE='||V_RUN_DATE
                          || ',Z.THRESHOLD_NUMBER='||Z.THRESHOLD_NUMBER
                          ||',Z.THRESHOLD_NUMBER_UNIT='||Z.THRESHOLD_NUMBER_UNIT );
      ...
      TOTAL_ERROR_COUNT := 0;
            ...
            LOOP
              ...
              CURR_DAY := X.FLAG_DATE;
               ...
               IF (    CONSECUTIVE_DAYS = TRUE    AND (CURR_DAY_ERRCOUNT >= Z.THRESHOLD_NUMBER))
              THEN
              ...
              END IF;
               ...
               IF (FLAGGING_DAYS >= Z.THRESHOLD_NUMBER_UNIT AND (CURR_DAY_ERRCOUNT >= Z.THRESHOLD_NUMBER))
               THEN
                    FLAG := 'YES';
...
*/                                    
                        ...
                        THEN
                           SELECT   STDDEV (ERRORPCT) * Z.THRESHOLD_NUMBER
                            ...
                            WHERE     MODULE_SN = UPPER(Y.MODULE_SN)
                                  ...
                                  AND FLAG_DATE BETWEEN V_DATE_30TH AND TRUNC (D.FLAG_DATE);
                           ...
                           THEN
                              ...
                              THEN
                                 ...
                                 IF CONSEQ_COUNT >= Z.THRESHOLD_NUMBER_UNIT
                                 THEN
                                    FLAG := 'YES';
                                    IHN_VALUE := Z.THRESHOLD_ALERT;
   ...
   THEN
      DBMS_OUTPUT.PUT_LINE ('SD_LOW_VOLUME=' || '  ' || Z.THRESHOLD_NUMBER);
