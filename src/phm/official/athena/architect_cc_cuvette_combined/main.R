...
tryCatch({
    ...
    #
    csv_params <- read_csv_file(filename="input.csv",
                                type_of_file="Parameters")
    csv_params$PHM_PATTERNS_SK_DUP <- csv_params$PHM_PATTERNS_SK
    param_sets <- split(csv_params, list(csv_params$PHM_PATTERNS_SK))
    #
    ...
    #
    ###############################################################################
    # NOTE - EXECUTE QUERY ON THE FIRST PARAMETER SET ONLY
    ###############################################################################
    #
    params <- param_sets[[1]]
    #
    ...
    if (nrow(flagged_results) == 0) {
        ...
        #
        # pass in the entire paramters set as a table.
        #
        flagged_results <- 
    ...
    if (nrow(modulesn_results) == 0) {
        ...
        if (nrow(not_flagged_results) > 0) {
            ...
            if (nrow(chart_data_results) > 0) {
                ...
                #
                not_flagged_results <- rbind(chart_data_not_flagged_results,
                                             no_chart_data_not_flagged_results)
            } else {
