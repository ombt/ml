...
#
get_low_today_test_count <- function()
{
...
#
get_low_today_errorpct <- function()
{
#                            -- GET TODAY_ERRORPCT
                                       ...
                                       threshold_data_days)
{
    ...
    for (idr in 1:nrow(data_results)) {
        ...
        write_data(current_sn_data_results, "sorted_current_sn_data_results.csv", TRUE)
        #
        ...
        for (isorted in 1:nrow(sorted_current_sn_data_results)) {
            ...
            if (flag_date_tstamp >= v_req_start_date_tstamp) {
                filter_count <- get_low_filter_count(current_sn_data_results, 
                                                     threshold_number_unit,
                                                     flag_date)
                if (filter_count > 0) {
                    v_date_30th <- get_low_v_date_30th()
                    today_test_count <- get_low_today_test_count()
                    today_errorpct <- get_low_today_errorpct()
                    #
                    flag <- 'no'
                    ihn_value <- ''
                    #
...
#
get_high_today_test_count <- function()
{
#                            -- Get TODAY_TEST_COUNT
#                            SELECT TESTCOUNT
#                              INTO TODAY_TEST_COUNT
...
#
get_high_today_errorpct <- function()
{
#                            -- GET TODAY_ERRORPCT
#                            SELECT NVL (ERRORPCT, 0)
#                              INTO TODAY_ERRORPCT
    ...
    for (idr in 1:nrow(data_results)) {
        ...
        for (isorted in 1:nrow(sorted_current_sn_data_results)) {
            ...
            flag_date_tstamp <- as.POSIXct(flag_date, 
                                           format="%Y-%m-%d %H:%M:%OS")
            if (flag_date_tstamp >= v_req_start_date_tstamp) {
                filter_count <- get_high_filter_count(current_sn_data_results, 
                                                      threshold_number_unit,
                                                      flag_date)
                if (filter_count > 0) {
                    v_date_30th <- get_high_v_date_30th()
                    today_test_count <- get_high_today_test_count()
                    today_errorpct <- get_high_today_errorpct()
                    #
                    flag <- 'no'
                    ihn_value <- ''
                    #
