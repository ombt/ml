                                     ...
                                     threshold_data_days)
{
    ...
    for (idr in 1:nrow(data_results)) {
        ...
        if (current_sn != dr_record[1,"A3600_SERIALNUMBER_UC"]) {
            print(sprintf("Processing SN: %s", dr_record[1,"A3600_SERIALNUMBER_UC"]))
            curr_day          <- NA
            prev_day          <- NA
            curr_day_tstamp   <- NA
            prev_day_tstamp   <- NA
        ...
        if ((flagging_days >= threshold_number_unit) &&
            ...
            #
            print(sprintf("SN: %s FLAGGED", dr_record[1,"A3600_SERIALNUMBER_UC"]))
        }
    }
    #
                                    ...
                                    threshold_data_days)
{
    ...
    for (idr in 1:nrow(data_results)) {
        dr_record <- data_results[idr, ]
        #
        write_data(dr_record, "percentage_data.csv")
        write_data(dr_record, "all_data.csv", TRUE)
        #
        if (nchar(dr_record[1,"A3600_SERIALNUMBER_UC"]) <= 0) {
            next
...
"
    ...
    if (errors$occurred()) {
        ...
        print(sprintf("No records found for V_REQ_START_DATE, %s", current_dn))
        return("")
    }
    names(results) <- toupper(names(results))
    write_data(results, "v_req_start_date.csv")
    write_data(results, "all_data.csv", TRUE)
    #
                                 ...
                                 flag_date_tstamp)
{
    ...
    #
    print(sprintf("get_low_filter_count: sn=%s, fc=%f,%d",
                  current_sn,
                  filter_count,
                                ...
                                flag_date_tstamp)
{
    ...
    #
    print(sprintf("get_low_v_date_30th: sn=%s, v_date_30th=%s",
                  current_sn,
                  v_date_30th))
    #
    return(v_date_30th)
                                     ...
                                     flag_date_tstamp)
{
    ...
    #
    print(sprintf("get_low_today_test_count: sn=%s, today_test_count=%d",
                  current_sn,
                  today_test_count))
    #
                                   ...
                                   flag_date_tstamp)
{
    ...
    today_errorpct <- as.integer(results[1,"ERRORPCT"])
    #
    print(sprintf("get_low_today_errorpct: sn=%s, today_errorpct=%f",
                  current_sn,
                  today_errorpct))
    #
...
#
get_low_v_theshold_limit <- function()
{
    return(0)
}
#
get_low_conseq_count <- function()
{
    return(0)
}
#
                                       ...
                                       threshold_data_days)
{
    ...
    for (idr in 1:nrow(data_results)) {
        dr_record <- data_results[idr, ]
        #
        write_data(dr_record, "sd_low_volume_data.csv")
        write_data(dr_record, "all_data.csv", TRUE)
        #
        if (nchar(dr_record[1,"A3600_SERIALNUMBER_UC"]) <= 0) {
            next
        ...
        current_pl <- dr_record[1,"A3600_IOM_PRODUCTLINE"]
        write_data(current_sn_data_results, "current_sn_data_results.csv", TRUE)
        #
        ...
        if (nchar(v_req_start_date) <= 0) {
            print(sprintf("Zero length V_REQ_START_DATE. Skipping %s.", current_sn))
            next
        } else {
            print(sprintf("%s V_REQ_START_DATE = <%s>", current_sn, v_req_start_date))
        }
        ...
        write_data(sorted_current_sn_data_results, 
                  "sorted_current_sn_data_results.csv", TRUE)
        #
        sorted_desc_current_sn_data_results <- 
            current_sn_data_results[order(current_sn_data_results$FLAG_DATE,
                                          decreasing=TRUE),]
        write_data(sorted_desc_current_sn_data_results, 
                  "sorted_desc_current_sn_data_results.csv", TRUE)
        #
        ...
        for (isorted in 1:nrow(sorted_current_sn_data_results)) {
            ...
            if (flag_date_tstamp >= v_req_start_date_tstamp) {
                filter_count <- get_low_filter_count(sorted_desc_current_sn_data_results, 
                                                     current_sn,
                                                     threshold_number_unit,
                                                     flag_date_tstamp)
                if (filter_count > 0) {
                    v_date_30th <- get_low_v_date_30th(sorted_desc_current_sn_data_results, 
                                                       current_sn,
                                                       threshold_number_unit,
                                                       flag_date_tstamp)
                    today_test_count <- 
                    ...
                    if (today_test_count >= filter_count) {
                        v_theshold_limit <- get_low_v_theshold_limit()
                        if (v_theshold_limit > 0) {
                            if (today_errorpct >= v_threshold_limit) {
                                conseq_count <- 0
                                flag <- 'no'
                                ihn_value <- ''
                                #
                                conseg_count <- get_low_conseq_count()
                                if (conseg_count >= threshold_number_unit) {
                                    ...
                                    flagged_results <- rbind(flagged_results,
                                                             ...
                                                             stringsAsFactors=FALSE)
                                    #
                                    print(sprintf("SN: %s FLAGGED", 
                                          dr_record[1,"A3600_SERIALNUMBER_UC"]))
                                } else {
                                    flag <- 'no'
                                    inh_value <- ''
                                }
                                  ...
                                  flag_date_tstamp)
{
    ...
    filter_count <- abs(mean(results$TUBESTODAY)-sd(results$TUBESTODAY))
    #
    print(sprintf("get_high_filter_count: sn=%s, fc=%f,%d",
                  current_sn,
                  filter_count,
                  as.integer(filter_count)))
    #
                                 ...
                                 flag_date_tstamp)
{
    ...
    v_date_30th <- min(results[,"FLAG_DATE"])
    #
    print(sprintf("get_high_v_date_30th: sn=%s, v_date_30th=%s",
                  current_sn,
                  v_date_30th))
    #
    return(v_date_30th)
                                      ...
                                      flag_date_tstamp)
{
    ...
    today_test_count <- as.integer(results[1,"TUBESTODAY"])
    #
    print(sprintf("get_high_today_test_count: sn=%s, today_test_count=%d",
                  current_sn,
                  today_test_count))
    #
    return(today_test_count)
                                    ...
                                    flag_date_tstamp)
{
    ...
    today_errorpct <- as.integer(results[1,"ERRORPCT"])
    #
    print(sprintf("get_high_today_errorpct: sn=%s, today_errorpct=%f",
                  current_sn,
                  today_errorpct))
    #
    return(today_errorpct)
...
#
get_high_v_theshold_limit <- function()
{
    return(0)
}
#
get_high_conseq_count <- function()
{
    return(0)
}
#
                                        ...
                                        threshold_data_days)
{
    ...
    for (idr in 1:nrow(data_results)) {
        dr_record <- data_results[idr, ]
        #
        write_data(dr_record, "sd_high_volume_data.csv")
        write_data(dr_record, "all_data.csv", TRUE)
        #
        if (nchar(dr_record[1,"A3600_SERIALNUMBER_UC"]) <= 0) {
            next
        ...
        for (isorted in 1:nrow(sorted_current_sn_data_results)) {
            ...
            flag_date_tstamp <- as.POSIXct(flag_date, 
                                           format="%Y-%m-%d %H:%M:%OS")
            if (flag_date_tstamp >= v_req_start_date_tstamp) {
                filter_count <- get_high_filter_count(sorted_desc_current_sn_data_results, 
                                                      current_sn,
                                                      threshold_number_unit,
                                                      flag_date_tstamp)
                if (filter_count > 0) {
                    v_date_30th <- get_high_v_date_30th(sorted_desc_current_sn_data_results, 
                                                        current_sn,
                                                        threshold_number_unit,
                                                        flag_date_tstamp)
                    today_test_count <- 
                    ...
                    #
                    if (today_test_count >= filter_count) {
                        v_theshold_limit <- get_high_v_theshold_limit()
                        if (v_theshold_limit > 0) {
                            if (today_errorpct >= v_threshold_limit) {
                                conseq_count <- 0
                                flag <- 'no'
                                ihn_value <- ''
                                #
                                conseg_count <- get_high_conseq_count()
                                if (conseg_count >= threshold_number_unit) {
                                    flag <- 'yes'
                                    inh_value <- threshold_alert
                                    v_flagged_pl <- NA
