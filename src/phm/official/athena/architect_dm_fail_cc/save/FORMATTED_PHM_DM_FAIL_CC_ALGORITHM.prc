CREATE OR REPLACE PROCEDURE SVC_PHM_OWNER.PHM_DM_FAIL_CC_algorithm(V_ALG_NUM NUMBER,
                                                                   V_RUN_DATE DATE,
...
IS
    ...
    BEGIN
        SELECT * BULK COLLECT INTO FLG_TBL 
        FROM (
            SELECT
                FINALDAT.DEVICEID,
                    ...
                    FROM (
                        ...
                        FROM (
                            SELECT 
                                *
                            FROM
                                SVC_PHM_ODS.PHM_ODS_DM_FAIL_CC MA
                            ORDER BY 
                                MA.MODULESNDRM,
        ...
        --DBMS_OUTPUT.PUT_LINE('DM_FAIL_CC_algorithm EXECUTION STARTED FOR : V_BATCH_NUM: ' || V_BATCH_NUM || ', V_RUN_DATE: ' || V_RUN_DATE);
        ...
        THEN
            ...
            THEN
                ...
                WHERE  
                    BATCH_NUM = V_BATCH_NUM 
                AND 
                    RUN_DATE = V_RUN_DATE AND PHM_PATTERNS_SK = V_ALG_NUM;
                COMMIT;
            END IF;
            ...
            LOOP
                ...
                LOOP
                    ...
                    THEN
                        SELECT
                            ...
                            COUNT(DISTINCT(TRUNC(CC.COMPLETIONDATE,'DDD'))) AS N_DAYS_WITH_PATIENT,
                            TRUNC(SYSDATE,'DDD') - TRUNC(MAX(CC.COMPLETIONDATE),'DDD') AS N_DAYS_SINCE_LAST_PAT
                        INTO 
                            V_N_PATIENT_SINCE_FAIL, 
                            V_N_DAYS_WITH_PATIENT, 
                            V_N_DAYS_SINCE_LAST_PAT
                        FROM
                            SVC_PHM_ODS.PHM_ODS_RESULTS_CC CC
                        WHERE
                            CC.MODULESNDRM = FLG_TBL (indx).MODULESNDRM 
                        AND
                            CC.SAMPLETYPE = 'PATIENT' 
                        AND
                            CC.COMPLETIONDATE >= FLG_TBL (indx).FIRST_FAIL;
                ...
                EXCEPTION
                WHEN OTHERS THEN
                    V_ERROR_MESSAGE :=  ' PHM_DM_FAIL_CC_algorithm EXECUTION HAS FAILED FOR '||V_ALG_NUM||' FOR '||DL.SERIAL_NUM||' FOR DATE '||V_FLAG_DATE_TIME||
', ERROR :'|| SQLERRM;
        ...
        WHEN OTHERS THEN
            V_PROCESS_STATUS := 'ERRORED';
            V_ERROR_MESSAGE  := 'ALGORITHM EXECUTION FAILED FOR DM_FAIL_CC_algorithm, DUE TO: ' || SQLERRM;
            PHM_ALGORITHM_UTILITIES_1.PHM_PROCESS_AUDIT_LOG(V_PROCESS_ID,
                                                            V_PROD_FAMILY,
                                                            V_PROCESS_TYPE,
                                                            V_ROUTINE_TYPE,
