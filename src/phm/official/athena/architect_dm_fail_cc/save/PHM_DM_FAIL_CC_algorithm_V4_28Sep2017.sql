CREATE OR REPLACE PROCEDURE SVC_PHM_OWNER.PHM_DM_FAIL_CC_algorithm(V_ALG_NUM NUMBER,V_RUN_DATE DATE,V_BATCH_NUM VARCHAR2, V_UNIX_ID VARCHAR2)
IS
                ...
                LOOP
                    IF FLG_TBL (indx).MODULESNDRM = DL.SERIAL_NUM THEN
                        ...
                        FROM
                            SVC_PHM_ODS.PHM_ODS_RESULTS_CC CC
                        WHERE
                            CC.MODULESNDRM = FLG_TBL (indx).MODULESNDRM AND
                            CC.SAMPLETYPE = 'PATIENT' AND
...
grant execute on SVC_PHM_OWNER.PHM_DM_FAIL_CC_algorithm to SVC_PHM_CONNECT;