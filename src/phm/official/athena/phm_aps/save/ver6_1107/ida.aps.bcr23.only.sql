...
BEGIN
  ...
  THEN
    ...
    SELECT 
        COUNT(*) INTO V_EXISTING_REC_COUNT 
    FROM 
        PHM_APS_DATA  
    WHERE 
        BATCH_NUM = V_BATCH_NUM;
    ...
    THEN
        ...
        DBMS_OUTPUT.PUT_LINE('Total instruments for the batch: ' || V_TOTAL_COUNT || ', Update/Insert count in PHM_APS_COUNTERS_TEMP -> V_UPDATE_COUNT: ' || V_UPDATE_COUNT || ', V_INSERT_COUNT: ' || V_INSERT_COUNT || ', TOTAL COUNT: ' || (V_UPDATE_COUNT + V_INSERT_COUNT));

        FOR TH IN ALL_THRESHOLDS (V_ALG_NUM)
        LOOP
             DBMS_OUTPUT.PUT_LINE('Pre-Processing PHM_PATTERNS_SK: ' || TH.PHM_PATTERNS_SK || ', PATTERN_NAME: ' || TH.PATTERN_NAME || ', ALGORITHM_TYPE: ' || TH.ALGORITHM_TYPE || ', PATTERN_TEXT: ' || TH.PATTERN_TEXT || ', ERROR_COUNT: ' || TH.ERROR_COUNT || ', IHN_LEVEL3_DESC: ' || TH.IHN_LEVEL3_DESC || ', THRESHOLD_DATA_DAYS: ' || TH.THRESHOLD_DATA_DAYS || ', THRESHOLDS_DAYS: ' || TH.THRESHOLDS_DAYS || ', THRESHOLD_DESCRIPTION: ' || TH.THRESHOLD_DESCRIPTION || ', THRESHOLD_TYPE: ' || TH.THRESHOLD_TYPE);    
             
            IF TH.ALGORITHM_TYPE = 'BCR_1'  
            THEN
                V_INSERT_COUNT := 0; 
                FOR X IN CURAPS_DEVICES_DATES
                LOOP
                    ...
                    FOR Y IN CURAPS_ERRORS_BCR1 (X.SN, 
                                                 VSTARTDATE, 
                                                 X.DT_MAX)
                    LOOP
                        VCOUNT_NORMAL_MIN    := 0;
                        ...
                        BEGIN
                            SELECT 
                                MAX_VALUE INTO VCOUNT_NORMAL_MIN 
                            FROM (
                                SELECT 
                                    *  
                                FROM 
                        ...
                        BEGIN
                            SELECT 
                                MAX_VALUE INTO VCOUNT_NORMAL_MAX 
                            FROM 
                                PHM_APS_COUNTERS_TEMP 
                            WHERE 
                                ID =  'normal' 
                        ...
                        BEGIN
                            SELECT 
                                MAX_VALUE INTO VCOUNT_PRIORITY_MIN 
                            FROM ( 
                                SELECT 
                                    *  
                                FROM 
                        ...
                        BEGIN
                            SELECT 
                                MAX_VALUE INTO VCOUNT_PRIORITY_MAX 
                            FROM 
                                PHM_APS_COUNTERS_TEMP 
                            WHERE 
                                ID =  'priority' 
                        ...
                        DBMS_OUTPUT.PUT_LINE('VCOUNT_NORMAL_MIN: ' || VCOUNT_NORMAL_MIN || ', VCOUNT_NORMAL_MAX: ' || VCOUNT_NORMAL_MAX  || ', VCOUNT_PRIORITY_MIN: ' || VCOUNT_PRIORITY_MIN || ', VCOUNT_PRIORITY_MAX: ' || VCOUNT_PRIORITY_MAX || ', Y.SN: ' || Y.SN || ', Y.DT: ' || Y.DT);
                        ...
                        DBMS_OUTPUT.PUT_LINE('Inserting BCR_1 data into PHM_APS_DATA for PL: ' || Y.PL || ', Y.SN: ' || Y.SN || ', PATTERN: ' || TH.PATTERN_TEXT || ', Y.FLG_DATE: ' || Y.FLG_DATE || ', ERRORS_PER_DAY: ' || ERRORS_PER_DAY || ', Y.PAT_ERRCOUNT: ' || Y.PAT_ERRCOUNT || ', PER_ERROR_COUNT: ' || PER_ERROR_COUNT);                      
            ...
            THEN
                ...
                LOOP
                    ...
                    FOR X IN CURAPS_DEVICE_LAS (Y.SN, 
                                                VSTARTDATE, 
                                                Y.DT_MAX, 
                                                TH.PATTERN_TEXT)
                    LOOP
                        ...
                        BEGIN
                            ...
                            FROM (
                                ...
                                FROM 
                                    PHM_APS_COUNTERS_TEMP 
                                WHERE 
                                    ID = SUBSTR(TH.PATTERN_TEXT, 4, 4) 
                                AND 
                                    DURATION = 'YTD' 
                                AND 
                                    DESCRIPTION = 'InstrumentBufferCounter' 
                                AND 
                                    SN = X.IOM_SN 
                                AND 
                                    TIMESTAMP <= X.DT 
                        ...
                        THEN
                            LASX_PERCENTAGE := 0; 
                            ERRORS_PER_DAY := 0;
                            V_ERROR_MESSAGE := 'NOT ABLE TO CALCULATE LAS PERCENTAGE OR NOT ABLE TO INSERT DATA INTO PHM_APS_DATA FOR ' || X.SN || ' FOR DATE ' || X.MAX_TIMESTAMP ||' FOR ' || VALGNAME || ', WITH ERROR :' || SQLERRM;
                            --PHM_ALGORITHM_UTILITIES_1.PHM_ALGORITHM_RUNLOG  (V_PROCESS_ID, V_ALG_NUM, V_RUN_DATE, V_BATCH_NUM, V_ERROR_MESSAGE, VALGNAME);
                        END;
                          
                        DBMS_OUTPUT.PUT_LINE('Inserting LAS_205 data into PHM_APS_DATA for PL: ' || X.PL || ', IOM_SN: ' || X.IOM_SN || ', X.SN: ' || X.SN || ', PATTERN: ' || TH.PATTERN_TEXT || ', X.MS_TIME: ' || X.MAX_TIMESTAMP || ', ERRORS_PER_DAY: ' || ERRORS_PER_DAY || ', X.LAS_ERROR_COUNT: ' || X.LAS_ERROR_COUNT || ', LASX_PERCENTAGE: ' || LASX_PERCENTAGE);                      
            ...
            THEN
                ...
                LOOP
                    ...
                    EXCEPTION WHEN OTHERS 
                    THEN
                        V_ERROR_MESSAGE := 'NOT ABLE TO CALCULATE PER_DAY_ERROR_COUNT OR NOT ABLE TO INSERT DATA INTO PHM_APS_DATA FOR ' || Y.IOM_SN || ' FOR DATE ' || Y.FLAG_DATE ||' FOR ' || VALGNAME || ', WITH ERROR :'|| SQLERRM;
            ...
            LOOP
                FOR REM_DATES IN (
                    SELECT DISTINCT 
                        TRUNC(DATE_ON_LOG_FILE) DT, 
                        SN, 
                        LFD.DEVICE_ID
                    FROM 
                        APS_LOG_FILE_DETAILS LFD, 
                        APS_LOG_FILES LF 
                    WHERE 
                        LF.ZIPID = LFD.ZIPID 
                    AND 
                        LF.DEVICE_ID = LFD.DEVICE_ID
                    AND 
                        LF.SN = X.SN 
                    AND 
                        TRUNC(DATE_ON_LOG_FILE) BETWEEN 
                            X.DT - TH.THRESHOLDS_DAYS 
                        AND 
                            X.DT_MAX
                    )
                LOOP
                    SELECT 
                        COUNT(*) INTO TH_DT_COUNT 
                    FROM 
                        PHM_APS_DATA 
                    WHERE 
                        PHM_PATTERNS_SK = TH.PHM_PATTERNS_SK 
                    AND 
                        IOM_SN = REM_DATES.SN
                    AND 
                        TRUNC(TIMESTAMP) = REM_DATES.DT 
                    AND 
                        BATCH_NUM = V_BATCH_NUM 
                    AND 
                        RUN_DATE = V_RUN_DATE;
                    ...
                    THEN
                        ...
                        LOOP
                            BEGIN
                                ...
                                SELECT 
                                    TRUNC(ABS(AVG(TESTCOUNT) - STDDEV(TESTCOUNT)), 0), 
                                    MIN(TIMESTAMP) 
                                INTO 
                                ...
                                FROM (
                                    ...
                                    AND 
                                        RUN_DATE = V_RUN_DATE 
                                    AND 
                                        TRUNC(TIMESTAMP) <= TRUNC(D.TIMESTAMP) 
                                    ORDER BY 
                                        TIMESTAMP DESC
                                )
                                WHERE 
                                    ROWNUM <= Z.THRESHOLD_DATA_DAYS;
                                ...
                                THEN
                                    ...
                                    SELECT 
                                        (STDDEV (ERRORPCT) * Z.ERROR_COUNT) + AVG (ERRORPCT) 
                                    INTO 
                                    ...
                                    AND 
                                        RUN_DATE = V_RUN_DATE 
                                    AND 
                                        TESTCOUNT >= ONE_SD_BELOW_MEAN_COUNT 
                                    AND 
                                        TRUNC(TIMESTAMP) BETWEEN 
                                            V_30DAYS_MIN_DATE 
                                        AND 
                                    ...
                                    THEN
                                        ...
                                        FOR I IN (
                                            ...
                                            WHERE 
                                                ROWNUM <=  Z.THRESHOLDS_DAYS 
                                            ORDER BY 
                                                TIMESTAMP DESC
                                            )
                                        LOOP
                                            IF I.ERRORPCT >= SD15_ABOVE_30MEAN_ERRORPCT 
                                            THEN
                                        ...
                                        IF CONSEQ_COUNT >= Z.THRESHOLDS_DAYS 
                                        THEN
                                            FLAG := 'YES';
                            ...
                            DBMS_OUTPUT.PUT_LINE('SN: ' || Y.SN || ', D.TIMESTAMP: ' || D.TIMESTAMP || ', ONE_SD_BELOW_MEAN_COUNT: ' ||  ONE_SD_BELOW_MEAN_COUNT || ', V_30DAYS_MIN_DATE: ' || V_30DAYS_MIN_DATE || ', TODAY_TEST_COUNT: ' || TODAY_TEST_COUNT || ', TODAY_ERRORPCT: ' || TODAY_ERRORPCT || ', SD15_ABOVE_30MEAN_ERRORPCT: ' || SD15_ABOVE_30MEAN_ERRORPCT || ', CONSEQ_COUNT: ' || CONSEQ_COUNT || ', Z.THRESHOLDS_DAYS: ' || Z.THRESHOLDS_DAYS || ', FLAG: ' || FLAG || ', IHN_VALUE: ' || IHN_VALUE || ', V_FLAGGED_PL: ' || V_FLAGGED_PL || ', V_FLAGGED_EXP_CODE: ' || V_FLAGGED_EXP_CODE);  
                         
                            PHM_ALGORITHM_UTILITIES_1.PHM_ALGORITHM_OUTPUT_IN_EXP_PL(V_CUSTOMER, 
                                                                                     V_CUST_NUM, 
                                                                                     D.DEVICE_ID, 
                                                                                     Y.SN, 
                                                                                     V_COUNTRY, 
            ...
            THEN
                ...
                LOOP
                    ...
                    LOOP
                        BEGIN
                            ...
                            THEN
                                ...
                                THEN
                                    FLAG := 'YES';
                                    IHN_VALUE :=  Z.ISSUE_DESCRIPTION;  --Z.THRESHOLD_ALERT;
                                    PHM_ALGORITHM_UTILITIES_1.PHM_GET_PL_EXP_CODE(Z.PHM_PATTERNS_SK, 
                                                                                  Y.PL, NULL, 
                                                                                  V_FLAGGED_PL, 
                                                                                  V_FLAGGED_EXP_CODE);
                                END IF;
                        ...
                        DBMS_OUTPUT.PUT_LINE('THRESHOLD_TYPE: ' || Z.THRESHOLD_TYPE || ', SN: ' || Y.SN || ', TIMESTAMP: ' ||  X.TIMESTAMP || ', CURR_DAY: ' || CURR_DAY || ', PREV_DAY: ' || PREV_DAY || ', CONSECUTIVE_DAYS: ' || (CASE WHEN (CONSECUTIVE_DAYS = TRUE) THEN 'TRUE'  ELSE 'FALSE' END) || ', TODAY_ERRORPCT: ' || TODAY_ERRORPCT || ', FLAG: ' || FLAG || ', IHN_VALUE: ' || IHN_VALUE || ', V_FLAGGED_PL: ' || V_FLAGGED_PL || ', V_FLAGGED_EXP_CODE: ' || V_FLAGGED_EXP_CODE);  
                        PHM_ALGORITHM_UTILITIES_1.PHM_ALGORITHM_OUTPUT_IN_EXP_PL(Y.CUSTOMER, 
                                                                                 Y.CUSTOMER_NUM, 
                                                                                 Y.DEVICE_ID, 
                                                                                 Y.SN, 
                ...
                LOOP
                    BEGIN
                        ...
                        THEN
                            ...
                            IF (CONSECUTIVE_DAYS = TRUE AND (CURR_DAY_ERRCOUNT >= Z.ERROR_COUNT)) 
                            THEN
                                FLAGGING_DAYS := FLAGGING_DAYS + 1;
                            ELSE
                                FLAGGING_DAYS := 0;                        
                            ...
                            DBMS_OUTPUT.PUT_LINE('THRESHOLD_TYPE: ' || Z.THRESHOLD_TYPE || ', SN: ' || Y.SN || ', TIMESTAMP: ' ||  X.TIMESTAMP || ', CURR_DAY: ' || CURR_DAY || ', PREV_DAY: ' || PREV_DAY || ', CONSECUTIVE_DAYS: ' || (CASE WHEN (CONSECUTIVE_DAYS = TRUE) THEN 'TRUE'  ELSE 'FALSE' END) || ', ERROR_COUNT: ' || CURR_DAY_ERRCOUNT || ', FLAG: ' || FLAG || ', IHN_VALUE: ' || IHN_VALUE || ', V_FLAGGED_PL: ' || V_FLAGGED_PL || ', V_FLAGGED_EXP_CODE: ' || V_FLAGGED_EXP_CODE);  
    
                            -- count based scenario Ex: 3 errors in a week, 1 error per day
                        ELSIF (Z.THRESHOLD_TYPE = 'COUNT') 
                        THEN
                            CURR_DAY_ERRCOUNT := X.ERRORCOUNT;
                            TOTAL_ERROR_COUNT := TOTAL_ERROR_COUNT + CURR_DAY_ERRCOUNT;
                            IF (TOTAL_ERROR_COUNT >= Z.ERROR_COUNT) 
                            THEN
                                FLAG := 'YES';
                                IHN_VALUE :=  Z.ISSUE_DESCRIPTION;  --Z.THRESHOLD_ALERT;
                                PHM_ALGORITHM_UTILITIES_1.PHM_GET_PL_EXP_CODE(Z.PHM_PATTERNS_SK, 
                            ...
                            DBMS_OUTPUT.PUT_LINE('THRESHOLD_TYPE: ' || Z.THRESHOLD_TYPE || ', SN: ' || Y.SN || ', TIMESTAMP: ' ||  X.TIMESTAMP || ', X.ERRORCOUNT: ' || X.ERRORCOUNT || ', TOTAL_ERROR_COUNT: ' || TOTAL_ERROR_COUNT || ', FLAG: ' || FLAG || ', IHN_VALUE: ' || IHN_VALUE || ', V_FLAGGED_PL: ' || V_FLAGGED_PL || ', V_FLAGGED_EXP_CODE: ' || V_FLAGGED_EXP_CODE);  
    
                            -- discreate days count scenario Ex: 1/day 3 days with errors in one week
                        ELSIF (Z.THRESHOLD_TYPE = 'DISCRETE') 
                        THEN
                            CURR_DAY_ERRCOUNT := X.ERRORCOUNT;
                            IF (CURR_DAY_ERRCOUNT >= Z.ERROR_COUNT) 
                            THEN
                                FLAGGING_DAYS := FLAGGING_DAYS + 1;
                            END IF;
                            
                            IF (FLAGGING_DAYS >= Z.THRESHOLD_DATA_DAYS) 
                            THEN
                                FLAG := 'YES';
                                IHN_VALUE :=  Z.ISSUE_DESCRIPTION;  --Z.THRESHOLD_ALERT;
                                PHM_ALGORITHM_UTILITIES_1.PHM_GET_PL_EXP_CODE(Z.PHM_PATTERNS_SK, 
                            ...
                            DBMS_OUTPUT.PUT_LINE('THRESHOLD_TYPE: ' || Z.THRESHOLD_TYPE || ', SN: ' || Y.SN || ', TIMESTAMP: ' ||  X.TIMESTAMP || ', CURR_DAY_ERRCOUNT: ' || CURR_DAY_ERRCOUNT || ', FLAGGING_DAYS: ' || FLAGGING_DAYS || ', FLAG: ' || FLAG || ', IHN_VALUE: ' || IHN_VALUE || ', V_FLAGGED_PL: ' || V_FLAGGED_PL || ', V_FLAGGED_EXP_CODE: ' || V_FLAGGED_EXP_CODE);  
    ...
    IF V_PROCESS_STATUS NOT IN ( 'ERRORED') 
    THEN
