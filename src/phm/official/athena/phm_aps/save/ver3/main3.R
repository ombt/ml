...
#
get_errors_bcr1_query <- function(sn, vstartdate, dt_max)
{
    query <- "
select
                          ...
                          test_period)
{
    ...
    if (algorithm_type == "BCR_1") {
        ...
        for (iddr in 1:nrow(dd_results)) {
            ...
            sn     <- dd_record[1,"SN"]
            dt     <- dd_record[1,"DT"]
            dt_max <- dd_record[1,"DT_MAX"]
            #
            dt_tstamp         <- as.POSIXct(dt, format="%Y-%m-%d %H:%M:%OS")
            vstartdate_tstamp <- dt_tstamp - as.integer(threshold_data_days)
            vstartdate        <- format(vstartdate_tstamp, "%Y-%m-%d %H:%M:%OS")
            #
            errors_bcr1_query <- get_errors_bcr1_query(sn, vstartdate, dt_max)
            ...
            #
            if (errors$occurred()) {
                stop(sprintf("INFO: Error readinf Errors BCR1 data for %s", sn)) 
            } else if (nrow(apsc_results) <= 0) {
            ...
            #
            print(sprintf("INFO: %d Errors BCR1 records found for %s",
                         nrow(ebcr1_results), sn))
        }
