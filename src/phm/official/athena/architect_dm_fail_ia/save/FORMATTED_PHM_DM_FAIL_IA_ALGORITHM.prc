CREATE OR REPLACE PROCEDURE SVC_PHM_OWNER.PHM_DM_FAIL_IA_algorithm(V_ALG_NUM NUMBER,
                                                                   V_RUN_DATE DATE,
...
IS
    ...
    SELECT 
        PP.PHM_ALGORITHM_DEFINITIONS_SK INTO V_ALG_DFN_SK 
    from 
        PHM_PATTERNS PP, 
    ...
    LOOP
        ...
        IF I.PARAMETER_NAME = 'DM_FAIL_DAYS_WITH_FAIL'
        THEN 
            T_N_DAYS_WITH_FAIL :=  I.PARAMETER_VALUES;  
        END IF;

        IF I.PARAMETER_NAME = 'DM_FAIL_N_PATIENT_SINCE_FAIL'       
        THEN 
            T_N_PATIENT_SINCE_FAIL :=  I.PARAMETER_VALUES;  
        END IF;

        IF I.PARAMETER_NAME = 'DM_FAIL_DAYS_WITH_PATIENT'          
        THEN 
            T_N_DAYS_WITH_PATIENT :=  I.PARAMETER_VALUES;  
        END IF;

        IF I.PARAMETER_NAME = 'DM_FAIL_DAYS_SINCE_LAST_PAT'        
        THEN 
            T_N_DAYS_SINCE_LAST_PAT :=  I.PARAMETER_VALUES;  
        END IF;
    ...
    THEN
        ...
        LOOP
                ...
                LOOP
                    IF FLG_TBL (indx).MODULESNDRM = DL.SERIAL_NUM THEN
                        ...
                        FROM
                            SVC_PHM_ODS.PHM_ODS_RESULTS_IA IA
                        WHERE
                            IA.MODULESNDRM = FLG_TBL (indx).MODULESNDRM 
                        AND
                            IA.SAMPLETYPE = 'PATIENT' 
                        AND
            ...
            EXCEPTION
            WHEN OTHERS THEN
                V_ERROR_MESSAGE :=  ' PHM_DM_FAIL_IA_algorithm EXECUTION HAS FAILED FOR '||V_ALG_NUM||' FOR '||DL.SERIAL_NUM||' FOR DATE '||V_FLAG_DATE_TIME|| ', ERROR :'|| SQLERRM;
