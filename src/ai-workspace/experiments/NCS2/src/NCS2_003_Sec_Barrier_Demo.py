...
def main():
    ...
    # Vehicle 
    model_xml_va = args.model_va
    model_bin_va = os.path.splitext(model_xml_va)[0] + ".bin"

    model_xml_lpr = args.model_lpr
    model_bin_lpr = os.path.splitext(model_xml_lpr)[0] + ".bin"
    ...
    net_va = IENetwork(model=model_xml_va, weights=model_bin_va)
    net_lpr = IENetwork(model=model_xml_lpr, weights=model_bin_lpr)
    
    
    log.info("DET - Net Loaded.")
    log.info( net.layers)

    log.info("ATTR - Net Loaded.")
    log.info( net_va.layers)
    
    log.info("READ - Net Loaded.")
    log.info( net_lpr.layers)
    
    # Step 3.1: Prepare the input area
    log.info("Preparing input blobs Det, Attr, Read {} {} {}".format(net.inputs, net_va.inputs, net_lpr.inputs))
    
    input_blob = next(iter(net.inputs))
    input_blob_va = next(iter(net_va.inputs))
    input_blob_lpr = next(iter(net_lpr.inputs))
    
    # Steo 3.2: Prepare the output area
    log.info("Preparing output blobs")
    out_blob = next(iter(net.outputs))
    out_blob_va = next(iter(net_va.outputs))
    out_blob_lpr = next(iter(net_lpr.outputs))
    
    # Step 3.3: Set the batch size of the network.
    # original 
    # net.batch_size = len(args.input)
    # net_va.batch_size = len(args.input)
    # net_lpr.batch_size = len(args.input)
    
    # Reset these to 1
    net.batch_size = 1
    net_va.batch_size = 1
    net_lpr.batch_size = 1
    ...
    if loadNet == 1:
        log.info("ATTR - Read Net inputs {} ".format(net_va.inputs[input_blob_va].shape) )
        n_va, c_va, h_va, w_va = net_va.inputs[input_blob_va].shape
        n, c, h, w = n_va, c_va, h_va, w_va

        # Step 4 - Loading model to plugin
        log.info("ATTR - Loading model to the plugin")
        exec_net_va = plugin.load(network=net_va)
        exec_net = exec_net_va
        input_blob = input_blob_va

    if loadNet == 2:
        log.info("READ - Read Net inputs {} ".format(net_lpr.inputs[input_blob_lpr].shape) )
        n_lpr, c_lpr, h_lpr, w_lpr = net_lpr.inputs[input_blob_lpr].shape
        n, c, h, w = n_lpr, c_lpr, h_lpr, w_lpr
        
        # Step 4: Loading model to the plugin
        log.info("READ - Loading model to the plugin")
        exec_net_lpr = plugin.load(network=net_lpr)
        exec_net = exec_net_lpr
        input_blob = input_blob_lpr
    ...
    while cap.isOpened():
        ...
        if exec_net.requests[cur_request_id].wait(-1) == 0:
            ...
            # Parse detection results of the current request
            if loadNet == 1:
                res = exec_net_va.requests[cur_request_id].outputs[out_blob_va]
                log.info("ATTR - Objects in result are {}".format(res[0]))
                                      
                for obj in res[0][0]:
                    # Draw only objects when probability more than specified threshold
                                        
                    if obj[0] > args.prob_threshold:
            
                        log.info("ATTR - Result found - {}".format(obj[0]))
                        """         
                        xmin = int(obj[3] * initial_w)
                        ymin = int(obj[4] * initial_h)
                        xmax = int(obj[5] * initial_w)
                        ...
                        """
                        color = (200, 10, 50)
                        cv2.rectangle(frame, (100, 100), (150, 150), color, 2)
                        det_label = labels_map[1] if labels_map else str(1)
                                    
            # Parse detection results of the current request
            if loadNet == 2:
                res = exec_net_lpr.requests[cur_request_id].outputs[out_blob_lpr]
                log.info("READ - Objects in result are {}".format(res[0][0]))
        ...
        person_counter_message = "Total Num of People Found: {} ".format(str(nextObjectID))
